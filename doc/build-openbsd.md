...
This guide describes how to build smoked and command-line utilities on OpenBSD.
...
The default C++ compiler that comes with OpenBSD 5.9 is g++ 4.2. This version is old (from 2007), and is not able to compile the current version of Smoke, primarily as it has no C++11 support, but even before there were issues. So here we will be installing a newer compiler.
...
This makes it necessary to build boost, or at least the parts used by Smoke, manually:

```
# Pick some path to install boost to, here we create a directory within the smoke directory
BITCOIN_ROOT=$(pwd)
...
# Build w/ minimum configuration necessary for smoke
