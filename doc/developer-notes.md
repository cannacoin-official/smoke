...
- [Development guidelines](#development-guidelines)
    - [General Smoke](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Smoke uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Smoke is a multi-threaded application, and deadlocks or other
...
Smoke can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Smoke, where everyone uses
...
pay attention to for reviewers of Smoke code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Smoke uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Smoke, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Smoke so that they can be integrated
...
file descriptors, it will cause problems with Smoke's `select()` loop, because
...
compatibility between nodes. This happened in Smoke 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Smoke's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Smoke, the wallet was always
